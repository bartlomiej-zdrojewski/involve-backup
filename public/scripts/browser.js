...
Involve.controller( 'BrowserController', [ '$scope', '$window', '$http', '$interval', '$sce', '$mdDialog', '$mdToast', 'server', function ( $scope, $window, $http, $interval, $sce, $mdDialog, $mdToast, server ) {
        ...
        if ( $scope.SearchPhrase.length >= 3 ) {
            ...
            for ( let i = 0; i < $scope.projects.length; i++ ) {
                ...
                if ( !( $scope.FilterF || $scope.projects[i].filters[5] == false ) ) {
                    ...
                    continue; }
                    */
                if ( $scope.projects[i].title.toLowerCase().indexOf( $scope.SearchPhrase.toLowerCase() ) != -1 || $scope.projects[i].depiction.toLowerCase().indexOf( $scope.SearchPhrase.toLowerCase() ) != -1 ) {
        ...
        $http.get( server.url + server.api + "/browser" ).then(
            ...
            function ( response ) {
                ...
                if ( response.data.tutorial ) {
                    ...
                    $http.get( server.url + "/tutorials/browser.json" ).then(
