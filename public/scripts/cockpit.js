...
Involve.controller( 'CockpitController', [ '$scope', '$window', '$http', '$interval', '$sce', '$mdDialog', '$mdToast', 'server', function ( $scope, $window, $http, $interval, $sce, $mdDialog, $mdToast, server ) {

    $scope.CardsState = true;
    $scope.Tutorial = {};
    $scope.TutorialData = {};
    ...
    $scope.CreateProject = function ( event ) {
        ...
        $http.post( server.url + server.api + '/create_project', {

            title: $scope.ProjectCreation.title,
            depiction: $scope.ProjectCreation.depiction
                        
            } ).then(
                
                function ( response ) {

                    $window.location.href = "project.html?project=" + response.data.hash;

                    },

                function ( response ) {

                    console.log( "ERROR #" + response.status + " IN CREATE_PROJECT: " + response.data );

                    $mdDialog.show(
                                    
                        $mdDialog.alert ( )
                                        
                            .title( 'Podczas tworzenia projektu wystąpił błąd' )
                            .textContent( response.data )
                            .ariaLabel( 'Podczas tworzenia projektu wystąpił błąd' )
                            .ok( 'Rozumiem' )
    ...
    $scope.ShowProjectPreview = function ( index, event ) {
        ...
        let Project = {
            ...
            applied: $scope.cards[index].applied,
    ...
    $scope.Setup = function ( ) {
        ...
        $http.get( server.url + server.api + "/cockpit" ).then(

            function ( response ) {

                $scope.cards = response.data.projects;
                $scope.notifications = response.data.notifications;

                if ( !$scope.cards.length ) {

                    $scope.CardsState = false; }
                ...
                if ( response.data.tutorial ) {
                    ...
                    $http.get( server.url + "/tutorials/cockpit.json" ).then(
    ...
    function ProjectPreviewController ( $scope, $mdDialog, $mdToast, server, project ) {
